...
path_to_data = Path(
    'C:/MOPGA 2022-2023 Dodji/ARGO float Angola/EcotaxaPart_BCG_UVP6/QuartileMiriamfromSpeedpython').expanduser()
...
path_to_SPEED = Path(
    'C:/MOPGA 2022-2023 Dodji/ARGO float Angola/EcotaxaPart_BCG_UVP6_Plot/ClusterCenterQselection/SPEED').expanduser()
...
#tp_lista = [[20210819, 20210925], [20211001, 20211113], [20211113, 20211215], [20220120, 20220306],[20220310, 20220416]]
...
# cluster from kmeans or umap ( distance to center)
Q = [5, 10, 20, 25, 30, 40, 50, 60, 70, 75, 80, 90, 95,100]
...
for q in Q:
#path_create = Path('C:/MOPGA 2022-2023 Dodji/ARGO float Angola/EcotaxaPart_BCG_UVP6_Plot/ClusterCenterQselection/SPEED' + 'Q' + str(q)).expanduser()
    ...
    for tp_list in tp_lista:
        ...
        for cl in range(1, 7):
            ...
            path_to_figures = Path(
                'C:/MOPGA 2022-2023 Dodji/ARGO float Angola/EcotaxaPart_BCG_UVP6_Plot/ClusterCenterQselection/SPEED/SPEED' + str(
                    cl)).expanduser()
            ...
            if q<101:
                ...
                # cluster kmeans and object id
                cluster0 = pd.read_csv('C:/MOPGA 2022-2023 Dodji/ARGO float Angola/EcotaxaPart_BCG_UVP6/k-means_pca_resultscENTER_Q' + str(q) + '.csv')
                Taille_cluster = np.size(cluster0[cluster0.Cluster == 'cluster 1']['Cluster'])/223283
                if q<100:
                    cluster0 = cluster0[cluster0.keep == True]

                cluster0.drop(['datetime', 'depth_min', 'Dim1', 'Dim2', 'Dim3', 'Dim4'], inplace=True, axis=1)
                cluster0.rename(columns={'id': 'object_id'}, inplace=True)
                # check if cluster5 EXIST
                ...
                if np.size(indiv) != 0:
                    ...
                    for tp in tp_list[0:-1]:
                        ...
                        ecopart_bins = [0.645, 0.813, 1.020, 1.290, 1.630, 2.050, 2.580, 3.250, 4.100, 5.160, 6.500,8.190, 10.300, 13.000, 16.400, 20.600, 26.000, 10e7]
                        ...
                        for t in taxa_list:
                            ...
                            if cl == 6:
                                pp = PdfPages(
                                    str(path_to_figures) + '\Ecotaxa_maximum_concentrations'  + 'Q' + str(q) + str(def_chosen) + '_'
                                                                                                                 ' All detritus ' + str(
                                ...
                                pp = PdfPages(
                                    str(path_to_figures) + '\Ecotaxa_maximum_concentrations'  + 'Q' + str(q) + str(
                                        def_chosen) + '_' + str(
                            ...
                            for b in bin_min_list[0:-1]:
                                ...
                                for d in depth_part_list:
                                    plt.clf()
                                    # d = depth_part_list[0]
                                    df_temp_dX = df_temp[df_temp["depth_part"] == d].reset_index(drop=True)
                                    df_temp_d = df_temp_dX.groupby(['date', 'depth_part', 'bin_min', 'bin_max']) \
                                        .agg({'conc': 'median'}) \
                                        .reset_index()
                                    #0/0

                                    if df_temp_d.shape[0] > 4:  # if we have a median value for at least 5 out of the n days
                                        ...
                                        try:
                                            ...
                                            #ar = np.empty((0, 2), int)
                                            #ar = np.append(ar, np.array([[SE, MSE, RMSE, Rsquared]]), axis=0)
                                        ...
                                        arr = np.append(arr, np.array([[t, b, bmax, d, max_curvey, std_curvex, Rsquared, day]]),
                                                        axis=0)
                                        day0
                                        # print(b,d)
                                    count_d += 1
                        ...
                        arr = pd.DataFrame(arr,
                                           columns=['taxa', 'bin_min', 'bin_max', 'depth', 'max_abun', 'std_curvex', 'R2gauss','day'])
                        ...
                        for t in taxa_list:
                            ...
                            for b in bin_min_list[0:-1]:
                                ...
                                arr_temp = arr_temp[arr_temp['depth'] < 500].reset_index(drop=True)
                                bmax = bin_min_list[count_b + 1]
                        ...
                        sinking_speed = pd.DataFrame(sinking_speed, columns=['definition', 'taxa', 'bin_min', 'bin_max',
                                                                             ...
                                                                             'attenuation_ci_sup',
                                                                             'Abundance_depthmin', 'attenuationR2'])
                        # save it

                        if cl == 6:
                            arr.to_csv(
                                str(path_to_SPEED) + '\Info_sinking_' + 'Q' + str(q) + def_chosen + ' ' + ' All detritus' + str(
                                    tp_list_char.date[count_tp]) + ' to ' + str(
                                    tp_list_char.date[count_tp + 1]) + '.csv',
                                index=False)
                            sinking_speed.to_csv(
                                str(path_to_figures) + '\Ecotaxa_sinking_speed_' + 'Q' + str(q) + def_chosen + ' ' + ' All detritus' + str(
                                    tp_list_char.date[count_tp]) + ' to ' + str(
                                    tp_list_char.date[count_tp + 1]) + '.csv',
                                index=False)
                            sinking_speed.to_csv(
                                str(path_to_SPEED) + '\Ecotaxa_sinking_speed_' + 'Q' + str(q) + def_chosen + ' ' + ' All detritus' + str(
                                    tp_list_char.date[count_tp]) + ' to ' + str(
                                    tp_list_char.date[count_tp + 1]) + '.csv',
                                index=False)
                        else:
                            arr.to_csv(
                                str(path_to_SPEED) + '\Info_sinking_' + 'Q' + str(q) + def_chosen + ' ' + ' Cluster' + str(cl) + str(
                                    tp_list_char.date[count_tp]) + ' to ' + str(
                                    tp_list_char.date[count_tp + 1]) + '.csv',
                                index=False)
                            sinking_speed.to_csv(
                                str(path_to_figures) + '\Ecotaxa_sinking_speed_' + 'Q' + str(q) + def_chosen + ' ' + ' Cluster' + str(
                                    cl) + str(
                                    tp_list_char.date[count_tp]) + ' to ' + str(
                                    tp_list_char.date[count_tp + 1]) + '.csv',
                                index=False)
                            sinking_speed.to_csv(
                                str(path_to_SPEED) + '\Ecotaxa_sinking_speed_' + 'Q' + str(q) + def_chosen + ' ' + ' Cluster' + str(
                                    cl) + str(
                                    tp_list_char.date[count_tp]) + ' to ' + str(
                                    tp_list_char.date[count_tp + 1]) + '.csv',
                                index=False)
                        ...
                        if cl == 6:
                            vitesses = pd.read_csv(
                                str(path_to_figures) + '\Ecotaxa_sinking_speed_' + 'Q' + str(q) + def_chosen + ' ' + ' All detritus' +
                        ...
                        if cl == 6:
                            pp = PdfPages(str(path_to_figures) + '\ vitesse_graphique_allsize' + 'Q' + str(q) + str(
                                def_chosen) + '_' + ' All detritus' + '; ' + str(
                                tp_list_char.date[count_tp]) + ' to ' + str(tp_list_char.date[count_tp + 1]) + '.pdf')
                        else:
                            pp = PdfPages(
                                str(path_to_figures) + '\ vitesse_graphique_allsize' + 'Q' + str(q) + str(def_chosen) + '_' + str(
            ...
            #aaa = [1 / 0]
        path_to_fig = Path(
            'C:/MOPGA 2022-2023 Dodji/ARGO float Angola/EcotaxaPart_BCG_UVP6_Plot/ClusterCenterQselection/SPEED/SPEED').expanduser()

        vitesses = pd.read_csv(
            str(path_to_fig) + '1' + '\Ecotaxa_sinking_speed_' + 'Q' + str(q) + def_chosen + ' ' + ' Cluster1' +
        ...
        vitesses = pd.read_csv(
            str(path_to_fig) + '2' + '\Ecotaxa_sinking_speed_' + 'Q' + str(q) + def_chosen + ' ' + ' Cluster2' +
            str(tp_list_char.date[0]) + ' to ' + str(tp_list_char.date[1]) + '.csv')
        ...
        vitesses = pd.read_csv(
            str(path_to_fig) + '3' + '\Ecotaxa_sinking_speed_' + 'Q' + str(q) + def_chosen + ' ' + ' Cluster3' +
            str(tp_list_char.date[0]) + ' to ' + str(tp_list_char.date[1]) + '.csv')
        ...
        vitesses = pd.read_csv(
            str(path_to_fig) + '4' + '\Ecotaxa_sinking_speed_' + 'Q' + str(q) + def_chosen + ' ' + ' Cluster4' +
            str(tp_list_char.date[0]) + ' to ' + str(tp_list_char.date[1]) + '.csv')
        ...
        if np.size(CL0) > 1:
            vitesses = pd.read_csv(
                str(path_to_fig) + '5' + '\Ecotaxa_sinking_speed_' + 'Q' + str(q) + def_chosen + ' ' + ' Cluster5' +
                str(tp_list_char.date[0]) + ' to ' + str(tp_list_char.date[1]) + '.csv')
        ...
        vitesses = pd.read_csv(
            str(path_to_fig) + '6' + '\Ecotaxa_sinking_speed_' + 'Q' + str(q) + def_chosen + ' ' + ' All detritus' +
            str(tp_list_char.date[0]) + ' to ' + str(tp_list_char.date[1]) + '.csv')
        st1 = vitesses.bin_min
        st2 = vitesses.bin_max
        st1s = st1.astype("string")
        ...
        pp = PdfPages(str(path_to_fig) + '\ vitesse_graphique_allsize' + 'Q' + str(q) + '_' + 'Detritus ' + ' Clusters ' + ' ' + str(
            tp_list_char.date[0]) + ' to ' + str(tp_list_char.date[1]) + '.pdf')

        '''x_vvit=x_vit1._append(x_vit2)
        x_vvit = x_vvit._append(x_vit3)
        x_vvit = x_vvit._append(x_vit4)
        x_vvit = x_vvit._append(x_vit6)
        x_vvit=np.unique(x_vvit)
        sx=np.size(x_vvit)
        SX=np.empty((1,sx))
        SX[:]=np.nan'''
        plt.clf()
...
#runpy.run_path('C:\MOPGA 2022-2023 Dodji\ARGO float Angola\codes scripts Alexandre_stage2023_mars/MDS_angola.py')
...
'''''def correctiondayspeed(x, DY):
    DY = pd.DataFrame(DY, columns=['day'])
    count_da = 0
    for da in DY['day']:
        if x == da:
            if int(x) == int(DY['day'][0]) & count_da == 0:
                dx = da
            else:
                nexto = int(DY['day'][0]) + count_da * 6
                if int(x) <= nexto & int(x) >= int(DY['day'][count_da - 1]):
                    dx = da
                else:
                    dx = np.nan
        count_da += 1
