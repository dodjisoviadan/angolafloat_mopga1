...
#************************* READ FIRST interpolation....._V_Janv2024_2025.py****#
#***********************************************************************#
...
for q in Q:
    ...
    for cl in range(1, 5):
        ...
        for t in taxa_list:
            ...
            if arg2 == 1:
                df['vol_log'] = df["vol_sph"].apply(lambda x: np.log1p(x))
            else:
                df = df
            ...
            if arg2 == 1:
                parameter_dict = {"vol_log": [min(df.vol_log), max(df.vol_log)]}
                legend_dict = {"vol_log": "Biovolume [$mm^{3}$.$m^{-3}$]"} #, log$_{e}$ scale
            else:
                'none'
            ...
            for p in parameter_dict.keys():
                ...
                contour_levels = contour_levels_func(min_contour_level, max_contour_level/5, levels)
                ...
                HIG=-2.7#HIG=500
                '''plt.text(xi[250], -HIG, '1', fontsize = 15, color='#ED7D31')
                ax1.text(xi[350], -HIG, '2', fontsize = 15, color='#ED7D31')
                ax1.text(xi[490], -HIG, '3', fontsize = 15, color='#ED7D31')
                ax1.text(xi[595], -HIG, '4', fontsize = 15, color='#ED7D31')
                ax1.text(xi[800], -HIG, '5', fontsize = 15, color='#ED7D31')
                ax1.text(xi[922], -HIG, '6', fontsize = 15, color='#ED7D31')'''
                ssi=25
                ...
                cb.ax.set_ylabel(legend_dict[parameter], fontsize=13.5)
                ...
                path_to_figures2024 = Path('C:/Users/syawo/Downloads/Figures_pdf_Angola_Biogeosciences_submission2025/Interp_ClusterBiovolume').expanduser()
                #plt.savefig(str(path_to_figures2024) + '/' + str(cl) + 'Q' + str(q) + 'April2024.png', dpi=1200)
                plt.savefig(str(path_to_figures2024) + '/' + str(cl) + 'Q' + str(q) + 'April2025.png', dpi=300) #bbox_inches='tight', pad_inches=0
                pp.savefig(fig, dpi=1200)  # Save each figure in the pdf #plt.gcf()
