...
% vrainer=132*((d/10)^0.62); % d en mm
% log10vr= 0.62*log10(d/10) + log10(132);
XR=[-2.4495 1.4322];          
...
str = {
    ...
    '\color[rgb]{0.6350 0.0780 0.1840}Particle abundance'};

%figure
hold on
Nn=find(strcmp(opts,choice)==1);

for sp=(N+1)%sp=1:(N+1) % manage the loop
if sp==(N+1)
    iT=1:m;
...
plot(xr,yr,'-','color',[0.9290, 0.6940, 0.1250],'LineWidth',1.5)
plot(xn,yn,'-k','LineWidth',1.5)
plot(xpn,ypn,'--k','LineWidth',1.5)
...
xlabel 'Log10(Size Classes,mm)' %FontSize 16
ylabel 'attenuation of particle abundance'  %FontSize 16
xlim([0 3.5])
xlim([-2.5 1.5])
xlim([-1.1 0.55])
ylim([0 2.5])
title 'Attenation variation with size classes and Detritus or Clusters'
title 'Attenuation variation with size'
title ''

else
xlabel 'Log10(Speed m/d)'
ylabel 'attenuation of particle abundance'
xlim([0 3.5])
ylim([0 2.5])
...
if sp(1)==(N+1)
    ...
    if sp==1 & strcmp(choice,'speed')~=1
        % add text
         t = text(log10(0.5),1,str,'Position',[-1.94056783940832 1.83474727434081 0], 'fontweight', 'bold'); 
    elseif sp==1 & strcmp(choice,'speed')==1
        % add text
t = text(log10(0.5),log10(80),str,'Position',[-1.94056783940832 1.83474727434081 0], 'fontweight', 'bold');
...
f=gcf
exportgraphics(f,'Fev2024Attenuation_SpeedG.png','Resolution',1200)
exportgraphics(f,'Fev2024Attenuation_SpeedG.pdf','Resolution',1200)

%%

text(log10(0.5),log10(80),'OLS','Position',[-1.65885348540287 -0.572365825726559 0]);
text(log10(0.5),log10(80),'Hierarchical','Position',[-1.65885348540287 -0.572365825726559 0]);
text(log10(0.5),log10(80),'BETA=1.17','Position',[-1.65885348540287 -0.572365825726559 0]);
text(log10(0.5),log10(80),' From Cael et al.2020','Position',[-1.65885348540287 -0.572365825726559 0]);
text(log10(0.5),log10(80),' From Kriest et al.2022','Position',[-1.65885348540287 -0.572365825726559 0]);
