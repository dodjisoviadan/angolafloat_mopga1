...
for t in detrilist:
    ...
    for layer in ['0-100m', '100-200m', '200-300m', '300-400m', '400-500m']:  # ['0-100m', '100-300m', '300-600m', '600-1000m']:
        ...
        if len(layer_data) > 0:
            ...
            plt.subplots_adjust(hspace=0.2, wspace=0.2, top=0.885, bottom=0.15, left=0.135, right=0.9)
            ...
            ax.plot(taxon_data['date'], np.log1p(taxon_data['vol_sph_sum']), marker='o', color=COCOL[count_layer], linestyle='--',
                    label=layer,markersize=3.5, markeredgecolor='white', markeredgewidth=0.5)
            ...
            high = np.max(np.log1p(taxon_data['vol_sph_sum']))
            tt=taxon_data['date']
    ...
    ax.set_ylabel('Integrated biovolume of particles ($mm^{3}$.$m^{-2}$)', fontsize=21)
    #ax.set_ylabel('Integrated biovolume of particles ($mm^{3}$.$m^{-2}$), log$_{e}$', fontsize=21) # scale
    ax.set_xlabel('Date', fontsize=21)
    ax.tick_params(axis='x', labelsize=20)
    ax.tick_params(axis='y', labelsize=20)

    ticks = [float(t.get_text().replace('−', '-')) for t in ax.get_yticklabels()]
    expticks = np.exp(ticks) - 1
    ax.set_yticklabels(np.round(expticks, 1))  # vertically oriented colorbar
    ...
    path_to_figo = Path('C:/Users/syawo/Downloads/Figures_pdf_Angola_Biogeosciences_submission2025/Interp_ClusterBiovolume').expanduser()
    #plt.savefig(str(path_to_figures) + '/AprilAugust2024category_sum_concentration' + t + 'okone.png', dpi=1200)
    plt.savefig(str(path_to_figo) + '/Fig2biovBIS_LOGCHANGED_check' + '.png', dpi=300)
